...
export class EditSpeakerComponent implements OnInit {
  ...
  constructor(private dao: DaoService, private route: ActivatedRoute, private router: Router, private auth: AuthService) {
    ...
    this.dao.user(uid, (speaker) => {
      ...
      this.auth.currentUserUid().subscribe( (uid) =>{
        this.dao.user(uid, userLogged =>{
         if(userLogged.type <= speaker.type && userLogged.uid  != speaker.uid){
